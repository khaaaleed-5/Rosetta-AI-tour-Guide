...
To get the backend services running locally, follow these steps.
...
```bash
cp .env.example .env
# Edit .env with your preferred editor (e.g., nano .env or vim .env)
```
...
### 3. Set Up the Main Backend Service
...
Configure your PostgreSQL connection in a `.env` file in this directory.

---

## ▶️ Running the Services

You can run each FastAPI service individually:

**NLP Service:**

```bash
# In the nlp_service directory
...
```bash
# In the backend_service directory
...
You can interact directly with the NLP service using `curl` or any API client.

### Non-Streaming Response

```bash
curl -X POST http://localhost:8001/nlp/response \
-H "Content-Type: application/json" \
-d '{
    "query": "Who was the son of Akhenaten?",
    "detected_monuments": ["Statue of Akhenaten"],
    "history": [],
    "memory": ""
}'
```
...
```bash
curl -X POST http://localhost:8001/nlp/response/stream \
-H "Content-Type: application/json" \
-d '{
    "query": "Tell me about the Karnak Temple Complex."
}'
```

The response will be a stream of Server-Sent Events (`text/event-stream`).
